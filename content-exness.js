...
(() => {
  ...
  // Real click: React listens on synthetic mouse events, not just .click().
  // Dispatching the full mousedown/mouseup/click sequence makes the React
  // tree update reliably.
  ...
  // Find an MUI toggle button by its text content (e.g. "Market", "Pending").
  // Exness uses MuiToggleButton with autogenerated classes, so positional
  // selectors break; matching by visible text is stable.
  function findToggleButton(text) {
    const wanted = text.trim().toLowerCase();
    const buttons = document.querySelectorAll('button.MuiToggleButtonGroup-grouped, button.MuiToggleButton-root, button[class*="ToggleButton"]');
    for (const b of buttons) {
      if ((b.textContent || '').trim().toLowerCase() === wanted) return b;
    }
    return null;
  }
