...
export async function fetchExnessCandles({ tabId, accountId, symbol = 'XAUUSDr', timeFrameMin = 1, count = 200, price = 'bid' }) {
  ...
  // Exness's REST API requires a bearer JWT, not just cookies. realtime-hook
  // stashes the most recent token it sees on window.__exscalpAuthHeader.
  // We pull it inside the MAIN-world function so the fetch carries the
  // user's actual session, exactly like the page's own requests.
  const t0 = Date.now();
  let results;
  try {
    results = await chrome.scripting.executeScript({
      ...
      func: async (u) => {
        try {
          ...
          if (!auth) {
            return { ok: false, status: 0, text: 'no_auth_header_captured', noAuth: true };
          }
  ...
  if (!r || !r.ok) {
    if (r?.noAuth) {
      trace('exness_history', 'no_auth_yet', { hint: 'token not captured yet — pan the chart or wait for next API call' });
      return null;
    }
