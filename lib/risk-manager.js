...
// Account-type gate. Defaults to 'unknown' (which is allowed). Definitive
// 'live' detection comes from the captured /accounts/{id} response — if its
// email field ends with @exness.io or first_name matches a demo preset, we
// store 'demo'; everything else captured = 'live'.
//
// The URL alone is not enough — Exness serves both demo and live from the
// same /webtrading/ path; the difference is account-level, not URL-level.
export function detectAccountType(url) {
  // Reserved for future URL-based hints; currently URLs don't disambiguate.
  return 'unknown';
}

const DEMO_EMAIL_HOSTS = ['exness.io'];
const DEMO_NAME_HINTS  = [/^raw spread$/i, /^standard$/i, /^pro$/i, /^cent$/i, /^zero$/i];

export function classifyAccountFromApi(accountJson) {
  if (!accountJson || typeof accountJson !== 'object') return 'unknown';
  const email = accountJson?.personal?.email || '';
  const fname = (accountJson?.personal?.first_name || '').trim();
  if (DEMO_EMAIL_HOSTS.some(h => email.toLowerCase().endsWith('@' + h))) return 'demo';
  if (DEMO_NAME_HINTS.some(re => re.test(fname))) return 'demo';
  // Captured but not matching demo patterns → assume live
  return 'live';
}
