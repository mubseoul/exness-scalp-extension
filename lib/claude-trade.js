...
const COMMENTARY_SYSTEM = `You are a scalp-trading commentator watching XAUUSD on 1m bars.
There is NO breakout right now. In ONE short sentence each, tell the user:
  - Why no trade right now (what the price is doing inside the range).
  - What would trigger a long or short setup next.

Style: terse, concrete, professional. No hedging fluff. No emojis.
Respond with VALID JSON ONLY:
{"reason": "1 sentence", "watch": "1 sentence"}`;
...
Range (last 8 min): ${range?.low?.toFixed(2)} – ${range?.high?.toFixed(2)}  (width $${(range?.high - range?.low).toFixed(2)})
ATR(14): $${atr?.toFixed(2)}

Last 20 bars (UTC,O,H,L,C):
