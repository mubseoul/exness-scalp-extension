...
const MIN_WICK_PCT = 0.20;
const MAX_PRIOR_TOUCHES = 5;
...
export function detectZoneFade(bars1m, zones, atr) {
  ...
  // Wider SL on fades too — same reasoning as breakouts (noise on XAU 1m)
  ...
  for (const z of zones.filter(z => z.type === 'resistance')) {
    ...
    if (wickedIn && closedBelow && wickPct >= MIN_WICK_PCT && bearishBody) {
      ...
      const sl = round2(z.top + SL_ATR_MULT * atr);
      const tp = round2(entry - TP_ATR_MULT * atr);
      return {
        signal: {
          side: 'short',
          entry, sl, tp,
  ...
  for (const z of zones.filter(z => z.type === 'support')) {
    ...
    if (wickedIn && closedAbove && wickPct >= MIN_WICK_PCT && bullishBody) {
      const entry = round2(bar.c);
      const sl = round2(z.bottom - SL_ATR_MULT * atr);
      const tp = round2(entry + TP_ATR_MULT * atr);
      return {
        signal: {
          side: 'long',
