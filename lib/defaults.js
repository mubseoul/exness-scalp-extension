...
export const DEFAULTS = {
  ...
  // Strategy selector — 'breakout' / 'zone_fade' / 'both'
  // both = run both engines each cycle, take whichever fires first
  strategy: 'both',
  ...
  signal: {
    ...
    interval: '1m',           // 1m bars
    rangeMinutes: 8,          // breakout = price exits 8m high/low range
    breakoutAtrMult: 0.25,    // require breakout move >= 0.25 * ATR (stronger filter, fewer fakeouts)
    minAtrUsd: 0.40,          // skip if ATR(14) < $0.40
  ...
  risk: {
    ...
    tpAtrMult: 2.0,           // TP distance = N x ATR (wider — captures more of the move)
    slAtrMult: 1.5,           // SL distance = N x ATR (wider — survives normal noise on XAUUSD)
  ...
  // Trade management
  management: {
    beEnabled: true,          // move SL to entry once price hits 50% to TP
    beThreshold: 0.5,         // 0.5 = move BE at halfway to TP, 0.7 = 70% there, etc.
  },
  ...
  claude: {
    ...
    minConfidence: 65,        // skip if Claude's confidence < this (tighter — only conviction trades)
    timeoutSec: 12,           // give up on Claude after N sec, fall back to JS-only
    commentaryEveryMin: 5,    // narrate "why no trade" every N minutes when idle
  },
  ...
  state: {
    ...
    tradesThisHour: 0,
    lastSignalTs: 0,
    lastBarTs: 0,             // TIMESTAMP OF THE BAR (epoch ms of the candle)
    lastFetchOkTs: 0,         // WHEN we last successfully fetched bars
    lastFetchSource: null,    // 'XAUUSD=X' or 'GC=F' — which one served the data
    ...
    cycleCount: 0,            // monotonic counter, lifetime of this install
    openPositions: 0,
    pending: null,            // { id, side, entry, sl, tp, ts, ... } awaiting confirm
    history: [],              // recent signals/trades, capped 200
    exnessAccountId: null,    // sniffed from WS URL — needed for history fetch
    detectedAccountType: null,// 'demo' or 'live' — set by classifyAccountFromApi
    lastCommentaryAt: 0,      // when Claude last produced market commentary
    // P&L tracking (live, refreshed each cycle via /balance)
    currentBalance: null,
    currentEquity: null,
    startBalance: null,       // balance at start of UTC day
    balanceDayUtc: null,      // YYYY-MM-DD; rolls over each new UTC day
    todayPnL: 0,              // currentBalance - startBalance
    tradesPlacedToday: 0,     // count from history filtered to today
    openTrades: [],           // [{positionId, side, entry, sl, tp, atr, beMoved, placedAt}]
  },
};
