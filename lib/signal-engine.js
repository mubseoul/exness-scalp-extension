...
export function detectBreakout(bars, cfg) {
  // Defaults guard against missing keys — caller can pass just the signal
  // config without risk fields and we'll still compute sane SL/TP.
  const {
    rangeMinutes,
    breakoutAtrMult,
    minAtrUsd,
  ...
  // We need rangeMinutes for the breakout window + 14 for ATR(14) + 2 buffer.
  const minBars = rangeMinutes + 16;
  if (!Array.isArray(bars) || bars.length < minBars) {
    return { signal: null, reason: 'insufficient_bars', barCount: bars?.length || 0, minBars };
  }

  const nowUtcHour = new Date().getUTCHours();
  if (!cfg.ignoreSession && !inSession(nowUtcHour, sessionUtcHours)) {
